...
extern motor_measure_t motor_chassis[6];        //拉屎
int pitch_RPM;                                  //微分速度
int pitch_LastAng;                              //上次角度

int MiddleLimit = 700;
uint32_t PID_cal_LastTime = 0;
float GimbalPitch = 4700;
float GimbalYaw = 2000;
...
//    }
    ...
    //YQY沙箱
//    YQY_PID_Init(&YQY_PitchMotor);                          //pitch轴云台单独控制
    YQY_PitchMotor.kp = 120;
    YQY_PitchMotor.ki = 0;
    YQY_PitchMotor.kd = 0;
    YQY_PitchMotorAng.kp = 5;
    YQY_PitchMotorAng.ki = 0;
    YQY_PitchMotorAng.kd = 7;
    
    YQY_YawMotor.kp = 50;
    YQY_YawMotor.ki = 0;
    YQY_YawMotor.kd = 0;
    YQY_YawMotorAng.kp = 5;
    YQY_YawMotorAng.ki = 0;
    YQY_YawMotorAng.kd = 2;
    
    YQY_PitchMotorAng.target = 4400;
    YQY_YawMotorAng.target = 2000;
    
    while (1)
    { 
      if(HAL_GetTick() - PID_cal_LastTime >= 2)        //
      {
        PID_cal_LastTime = HAL_GetTick();
        //YQY_PitchMotorAng.target=4400+100*gimbal_control.gimbal_pitch_motor.absolute_angle_set;
        GimbalPitch += gimbal_control.gimbal_rc_ctrl->rc.ch[PITCH_CHANNEL] / 800.0;
        GimbalYaw   -= gimbal_control.gimbal_rc_ctrl->rc.ch[YAW_CHANNEL]   / 800.0;
        GimbalPitch += visionDataGet.pitch_angle.pitch_predict / 1000.0;
        GimbalYaw   += visionDataGet.yaw_angle.yaw_predict     / 1000.0;
        
        if(GimbalPitch > 5000)GimbalPitch = 5000;
        if(GimbalPitch < 4100)GimbalPitch = 4100;
        if(GimbalYaw > 3000)GimbalYaw = 3000;
        if(GimbalYaw < 1000)GimbalYaw = 1000;
        
        YQY_PitchMotorAng.target = GimbalPitch;
        YQY_YawMotorAng.target = GimbalYaw;
        
        YQY_PID_Cal(&YQY_PitchMotorAng, (float)motor_chassis[5].ecd);           //角度
        YQY_PitchMotor.target = YQY_PitchMotorAng.out;
        LimitMax(YQY_PitchMotor.target,MiddleLimit);
        YQY_PID_Cal(&YQY_PitchMotor,  INS_gyro[0] * 57.3);                      //速度
        
        YQY_PID_Cal(&YQY_YawMotorAng, (float)motor_chassis[4].ecd);             //角度
        YQY_YawMotor.target = YQY_YawMotorAng.out;
        LimitMax(YQY_YawMotor.target,MiddleLimit);
        YQY_PID_Cal(&YQY_YawMotor,  INS_gyro[2] * 57.3);                         //速度
        
        YQY_PitchMotor.out = YQY_Fliter(YQY_PitchMotor.out);
        YQY_YawMotor.out = YQY_FliterYaw(YQY_YawMotor.out);
        
        if(gimbal_control.gimbal_rc_ctrl->rc.s[0] == 3)
          CAN_cmd_gimbal(YQY_YawMotor.out, YQY_PitchMotor.out, 0);
        else
...
//			}
		  //Sendtosightway(16);
//			UI_ReFreshAll();
			Tidy_send_vision(&visionDataSend);                   //填充视觉信息
...
#else
        yaw_can_set_current = gimbal_control.gimbal_yaw_motor.given_current;        //这个
#endif

#if PITCH_TURN
        pitch_can_set_current = -gimbal_control.gimbal_pitch_motor.given_current;
#else
        pitch_can_set_current = gimbal_control.gimbal_pitch_motor.given_current;    //这个
#endif
...
//            {
//                CAN_cmd_gimbal(0, YQY_PitchMotor.out, 0);
//            }
//            else
//            {
//                CAN_cmd_gimbal(yaw_can_set_current, 0, shoot_can_set_current);
//            }
//        }
...
static void gimbal_init(gimbal_control_t *init)
{
	...
	//设置pitch轴软件限位
	init->gimbal_pitch_motor.max_software_relative_angle = (2400-init->gimbal_pitch_motor.offset_ecd) * (PI/HALF_ECD_RANGE);      //5060
	init->gimbal_pitch_motor.max_software_relative_angle_xtl = (2400-init->gimbal_pitch_motor.offset_ecd) * (PI/HALF_ECD_RANGE);; //4920
...
static void gimbal_motor_absolute_angle_control(gimbal_motor_t *gimbal_motor)
{
	...
	if(autoaim_mode_flag == 0)  //正常操作手模式
	{
		...
		if(gimbal_motor == &gimbal_control.gimbal_pitch_motor)
		{
			...
			kp_tmp += kp_min;
			gimbal_motor->gimbal_motor_absolute_angle_pid.kp = 25.0f;		         //28//26
			gimbal_motor->gimbal_motor_absolute_angle_pid.ki = 0.0f;              //0
			gimbal_motor->gimbal_motor_absolute_angle_pid.kd = 3.5f;            //0.3

			gimbal_motor->gimbal_motor_gyro_pid.Kp = 2500.0f;              //2175 
...
//				gimbal_motor->gimbal_motor_absolute_angle_pid.kp = 50.0f;		         //28//26低头
//				gimbal_motor->gimbal_motor_absolute_angle_pid.ki = 0.0f;              //0
//				gimbal_motor->gimbal_motor_absolute_angle_pid.kd = 0.0f;            //0.3

//				gimbal_motor->gimbal_motor_gyro_pid.Kp = 500.0f;              //2175 
//				gimbal_motor->gimbal_motor_gyro_pid.Ki = 2.0f;                 //5
//				gimbal_motor->gimbal_motor_gyro_pid.Kd = 0.0f;               //100
//			}
//			//pitch中值上参数：                                                        //抬头
//			else	{
//				gimbal_motor->gimbal_motor_absolute_angle_pid.kp = 10.0f;		        //10 //28//26
//				gimbal_motor->gimbal_motor_absolute_angle_pid.ki = 0.0f;              //0
//				gimbal_motor->gimbal_motor_absolute_angle_pid.kd = 0.3f;            //0.3

//				gimbal_motor->gimbal_motor_gyro_pid.Kp = 500.0f;             //1500 //2175 
//				gimbal_motor->gimbal_motor_gyro_pid.Ki = 1.0f;                 //1 //5
//				gimbal_motor->gimbal_motor_gyro_pid.Kd = 0.0f;               //100
//			}
		}
		//角度环，速度环串级pid调试
		...
		//控制值赋值
		if(gimbal_motor->current_set>10000.0f)
		{
		  gimbal_motor->current_set=10000.0f;
		}
