...
int SWITCH_R_FLAG;
int LAST_SWITCH_R_FLAG;
int SWITCH_FLAG;
int LAST_SWITCH_FLAG;
...
void shoot_init(void)
{
    ...
    static const fp32 Trigger_speed_pid[3] = {TRIGGER_ANGLE_PID_KP, TRIGGER_ANGLE_PID_KI, TRIGGER_ANGLE_PID_KD};
		static const fp32 Switch_angle_pid[3]={SWITCH_ANGLE_PID_KP,SWITCH_ANGLE_PID_KI,SWITCH_ANGLE_PID_KD};
		static const fp32 Switch_pid[3]={SWITCH_PID_KP,SWITCH_PID_KI,SWITCH_PID_KD};
		//摩擦轮PID参数
    ...
    //电机指针
    shoot_control.switch_motor_measure = get_switch_motor_measure_point();
    ...
    //初始化PID
		PID_init(&shoot_control.switch_motor_angle_pid,PID_POSITION,Switch_angle_pid,SWITCH_ANGLE_PID_MAX_OUT,SWITCH_ANGLE_PID_MAX_IOUT);
		PID_init(&shoot_control.switch_motor_pid,PID_POSITION,Switch_pid,SWITCH_PID_MAX_OUT,SWITCH_PID_MAX_IOUT);
    ...
    shoot_control.ecd_count = 0;
		shoot_control.switch_ecd_count = 0;
    shoot_control.angle = shoot_control.shoot_motor_measure->ecd * MOTOR_ECD_TO_ANGLE;
		shoot_control.switch_angle =shoot_control.switch_motor_measure->ecd*MOTOR_ECD_TO_ANGLE;
    shoot_control.given_current = 0;
		shoot_control.switch_given_current =0;
    shoot_control.move_flag = 0;
//    shoot_control.set_angle = shoot_control.angle;
    shoot_control.speed = 0.0f;
		shoot_control.switch_speed = 0.0f;
    shoot_control.speed_set = 0.0f;
		shoot_control.switch_speed_set=0.0f;
...
//			 }
    ...
    {
								shoot_control.fric1_speed_set = 0.0f;  //2.65
					      shoot_control.fric2_speed_set = 0.0f;  //2.65 
...
//					shoot_control.fric2_speed_set = 1.65 + KEY_TO_FRIC_SPEED_SEN * shoot_speed_add_little;
	//		}
		...
		if(SWITCH_FLAG == 1)
		{
			shoot_control.switch_angle_set = 0.0f;
			PID_calc(&shoot_control.switch_motor_angle_pid,shoot_control.switch_angle,shoot_control.switch_angle_set);
		  PID_calc(&shoot_control.switch_motor_pid,shoot_control.switch_speed,shoot_control.switch_speed_set);
			shoot_control.switch_given_current = (int16_t)(shoot_control.switch_motor_pid.out);
		}
		else if(SWITCH_FLAG == 0)
		{
			shoot_control.switch_angle_set = 2.0f;
			PID_calc(&shoot_control.switch_motor_angle_pid,shoot_control.switch_angle,shoot_control.switch_angle_set);
		  PID_calc(&shoot_control.switch_motor_pid,shoot_control.switch_speed,shoot_control.switch_speed_set);
			shoot_control.switch_given_current = (int16_t)(shoot_control.switch_motor_pid.out);
		}
		CAN_cmd_fric(shoot_control.fric1_given_current, shoot_control.fric2_given_current,0, shoot_control.switch_given_current);   //摩擦轮电机赋值。 
		CAN_cmd_trigger(shoot_control.given_current);
...
//--------------------------枪管切换电机状态设置----------------------------------------
		SWITCH_R_FLAG = shoot_control.shoot_rc->key.v & KEY_PRESSED_OFFSET_R;
		if(SWITCH_R_FLAG && !LAST_SWITCH_R_FLAG)
		{
			SWITCH_FLAG = !SWITCH_FLAG;
			LAST_SWITCH_FLAG = SWITCH_FLAG;
		}
		LAST_SWITCH_R_FLAG = SWITCH_R_FLAG;
}
...
static void shoot_feedback_update(void)
{
    ...
    shoot_control.speed = speed_fliter_3;
		...
		//类似于拨弹电机，枪管切换电机也进行滤波
		static fp32 switch_speed_fliter_1 = 0.0f;
		static fp32 switch_speed_fliter_2 = 0.0f;
		static fp32 switch_speed_fliter_3 = 0.0f;
		
		static const fp32 switch_fliter_num[3] = {1.725709860247969f, -0.75594777109163436f, 0.030237910843665373f};
		
		switch_speed_fliter_1 = switch_speed_fliter_2;
		switch_speed_fliter_2 = switch_speed_fliter_3;
		speed_fliter_3 = switch_speed_fliter_2 * switch_fliter_num[0]+switch_speed_fliter_1*switch_fliter_num[1]+(shoot_control.switch_motor_measure->speed_rpm*MOTOR_RPM_TO_SPEED)*switch_fliter_num[2];
		//摩擦轮电机数据更新
    ...
    if (shoot_control.switch_motor_measure->ecd - shoot_control.switch_motor_measure->last_ecd > HALF_ECD_RANGE)
    {
        shoot_control.switch_ecd_count--;
    }
    else if (shoot_control.switch_motor_measure->ecd - shoot_control.switch_motor_measure->last_ecd < -HALF_ECD_RANGE)
    {
        shoot_control.switch_ecd_count++;
    }
    ...
    else if (shoot_control.ecd_count == -FULL_COUNT)
    {
     ...
     if (shoot_control.switch_ecd_count == FULL_COUNT)
    {
        shoot_control.switch_ecd_count = -(FULL_COUNT - 1);
    }
    else if (shoot_control.switch_ecd_count == -FULL_COUNT)
    {
        shoot_control.switch_ecd_count = FULL_COUNT - 1;
    }
    //计算输出轴角度
    shoot_control.angle = (shoot_control.ecd_count * ECD_RANGE + shoot_control.shoot_motor_measure->ecd) * MOTOR_ECD_TO_ANGLE;
		shoot_control.switch_angle=(shoot_control.switch_ecd_count *ECD_RANGE +shoot_control.switch_motor_measure->ecd) *MOTOR_ECD_TO_ANGLE;
    //微动开关
...
//    {
//        //没到达一直设置旋转速度
//        shoot_control.trigger_speed_set = 1.2f;
//        trigger_motor_turn_back();
//    }
//    else 
//    {
...
//				
//    }
    shoot_control.trigger_speed_set = 2.5f;
    trigger_motor_turn_back();
}
